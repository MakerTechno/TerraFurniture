...
/**
 * 渲染生命周期钩子接口，用于在 {@link BaseFunctionalGeoBER} 的默认渲染逻辑前后插入自定义处理。
 *
 * <p>实现此接口可用于执行额外的渲染准备、状态修改、功能叠加或调试输出等操作，
 * 以增强或替代默认的 GeoBlockEntity 渲染行为。</p>
 *
 * <p>该接口设计为与 {@link BaseFunctionalGeoBER.Builder#addRenderHook(IRenderFunctionHook)} 配合使用，
 * 支持多个钩子按注册顺序执行（前序）与逆序回调（后序），确保渲染逻辑的可组合性与可控性。</p>
 *
